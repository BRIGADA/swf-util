...
#define TRAIT_ATTR_FINAL 			0x10	// Is used with Trait_Method, Trait_Getter and Trait_Setter. It marks a
											// method that cannot be overridden by a sub-class
#define TRAIT_ATTR_OVERRIDE			0x20 	// Is used with Trait_Method, Trait_Getter and Trait_Setter. It marks a
											// method that has been overridden in this class
#define TRAIT_ATTR_METADATA			0x40	// Is used to signal that the fields metadata_count and metadata follow the
...
#define INSTANCE_FLAG_SEALED        0x01	// The class is sealed: properties can not be dynamically added
											// to instances of the class.

#define INSTANCE_FLAG_FINAL 		0x02 	// The class is final: it cannot be a base class for any other
											// class.

#define INSTANCE_FLAG_INTERFACE 	0x04 	// The class is an interface.

#define INSTANCE_FLAG_PROTECTED_NS 	0x08 	// The class uses its protected namespace and the
...
/**
 * The exception_info entry is used to define the range of ActionScript 3.0 instructions over which a particular exception handler
 * is engaged.
 */
struct ABCException {
	uint32_t from;		// The starting position in the code field from which the exception is enabled.
	uint32_t to;		// The ending position in the code field after which the exception is disabled.
	uint32_t target;	// The position in the code field to which control should jump if an exception of type exc_type is
						// encountered while executing instructions that lie within the region [from, to] of the code field.
	uint32_t type; 		// An index into the string array of the constant pool that identifies the name of the type of exception
						// that is to be monitored during the reign of this handler. A value of zero means the any type (“*”) and
						// implies that this exception handler will catch any type of exception thrown.
	uint32_t varName;	// This index into the string array of the constant pool defines the name of the variable that is to
						// receive the exception object when the exception is thrown and control is transferred to target location.

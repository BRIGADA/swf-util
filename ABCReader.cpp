...
uint32_t ABCReader::pos() {
    return _cur - _start;
}

void ABCReader::pos(uint32_t value) {
    _cur = _start + value;
}
...
ABCFile* ABCReader::read() {
    ...
    result->cpool.ints.resize(readU30());
    if (result->cpool.ints.empty()) result->cpool.ints.resize(1);
    for (ABCSI32List::iterator a = result->cpool.ints.begin() + 1; a != result->cpool.ints.end(); ++a) {
        (*a) = readU32();
    }

    result->cpool.uints.resize(readU30());
    if (result->cpool.uints.empty()) result->cpool.uints.resize(1);
    for (std::vector<uint32_t>::iterator a = result->cpool.uints.begin() + 1; a != result->cpool.uints.end(); ++a) {
        (*a) = readU32();
    }

    result->cpool.doubles.resize(readU30());
    if (result->cpool.doubles.empty()) result->cpool.doubles.resize(1);
    for (ABCDoubleList::iterator a = result->cpool.doubles.begin() + 1; a != result->cpool.doubles.end(); ++a) {
        (*a) = readD64();
    }

    result->cpool.strings.resize(readU30());
    if (result->cpool.strings.empty()) result->cpool.strings.resize(1);
    for (ABCStringList::iterator a = result->cpool.strings.begin() + 1; a != result->cpool.strings.end(); ++a) {
        readString(*a);
    }

    result->cpool.namespaces.resize(readU30());
    if (result->cpool.namespaces.empty()) result->cpool.namespaces.resize(1);
    for (ABCNamespaceList::iterator a = result->cpool.namespaces.begin() + 1; a != result->cpool.namespaces.end(); ++a) {
        (*a).kind = readU8();
        (*a).name = readU30();
    }

    result->cpool.namespaceSets.resize(readU30());
    if (result->cpool.namespaceSets.empty()) result->cpool.namespaceSets.resize(1);
    for (ABCNamespaceSetList::iterator a = result->cpool.namespaceSets.begin() + 1; a != result->cpool.namespaceSets.end(); ++a) {
        (*a).resize(readU30());
    ...
    result->cpool.multinames.resize(readU30());
    if (result->cpool.multinames.empty()) result->cpool.multinames.resize(1);
    for (ABCMultinameList::iterator a = result->cpool.multinames.begin() + 1; a != result->cpool.multinames.end(); ++a) {
        (*a).kind = readU8();
        switch ((*a).kind) {
            case CONSTANT_QName:
